...
### 2.3 Сбор данных из макета Figma необхдимых для UI: Количество экранов, шрифты, цвета, картинки
    ...
    7. Добавим файл TestPlan.xctestplan. При помощи этого файла будем отслеживать покрытие тестами кода. Посмотреть инструкцию как сделать можно по ссылке Apple Developer Documentaion:
    https://developer.apple.com/documentation/xcode/determining-how-much-code-your-tests-cover
    7.1 При добавлении новых классов, структур перечислений необходимо при создании указывать в таргетах unit test или после создания в панели инспекторов во вкладке file inspector
    7.2 Посмотреть покрытие тестами можно после запуска тестов. Запустить тесты можно в панели наввигации во вкладке test navigator. Дале во вкладке report navigator -> coverage можно увидеть проыент покрытия тестами кода. Если кликнуть по coverage, то можно увидеть что конкретно не покрыто тестами.
...
### 2.4 Создадим структуру данных для UI на основе макета из Figma и на основе данных получаемых по сети при помощи JSON.
    ...
    5. Сделаем тестирование этой структуры Todo и для ErrorHandler
    Unit тесты будем делать в файле ToDo_VIPER_1_1Tests.swift
    
    1. Поскольку структуру Todo делали для получения данных по сети, в файле с тестами понадобится свойство с типом JSONDecoder
    2. Для тестирования понадобится тестовые данные в формате JSON. Возьмём первый объект из файла по ссылке выше и положим его в файл TestData.swift. Предварительно нужно создать такой файл и создадим его в папке Resources
    3. Добавим тест для Todo и проверим получение данных. 
    ...
    4. Сделаем тест для ErrorHandler show error message description
    
    После теста проверим code test coverage и увидим, что покрытие тестами равно 100% 
    
    5. Для получения всех todo необходимо создать еще одну структуру. Создадим для нее файл и добавим в таргеты UI test. 
    6. Для тестирования понадобится добавить JSON данные в TestData.swift аналогичние как приходят по сети.
    7. Напишем тест, убедимся что тест пройден
